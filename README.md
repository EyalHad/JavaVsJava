...
There are 3 Long Term Support (`LTS`)  Java versions, and the main focus will be on them.	- __Java 8, 11, 17__.
Each version come with a set of features some are an addition to tool set and some come to replace existing methods, we list some features but definitely not all, and we are not intend to, since it is out of scope for this paper.
